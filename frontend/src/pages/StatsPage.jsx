...
export default function StatsPage() {
  ...
  const barData = weekRows.map((r, i) => ({
    date: weekLabels[i] ?? r.date,
  ...
  return (
    <div className="page">
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        {/* Summary grid — key uses label which is unique */}
        ...
        {/* Best + Worst — key uses habit_id which is unique */}
        {bw && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
            <div className="card" style={{ padding: 14, borderTop: '3px solid var(--success)' }}>
              ...
              {(bw.best ?? []).slice(0, 3).map((h) => (
                <div key={`best-${h.habit_id}`} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <div className="card" style={{ padding: 14, borderTop: '3px solid var(--danger)' }}>
              ...
              {(bw.worst ?? []).slice(0, 3).map((h) => (
                <div key={`worst-${h.habit_id}`} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 18 }}>{h.icon}</span>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{h.name}</div>
        ...
        {(cats?.categories ?? []).length > 0 && (
          <div className="card" style={{ padding: 16, marginBottom: 16 }}>
            ...
            {cats.categories.map((cat) => {
              ...
              return (
                <div key={`cat-${cat.category}`} style={{ marginBottom: 14 }}>
        ...
        {/* Mood correlation — key uses mood which is unique per entry */}
        {(mood?.correlation ?? []).length > 0 && (
          <div className="card" style={{ padding: 16, marginBottom: 16 }}>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Mood vs Habit Completion</div>
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>Avg habits completed per mood</div>
            {mood.correlation.map((m) => (
              <div key={`mood-${m.mood}`} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
        ...
        <div className="card" style={{ padding: 20, marginBottom: 16, background: 'linear-gradient(135deg, #6C63FF11, #FF658411)', border: '1px solid #6C63FF33' }}>
          ...
          <div style={{ fontSize: 15, lineHeight: 1.7 }}>
            {bw?.best?.[0]
              ? `You're most consistent with "${bw.best[0].name}" (${bw.best[0].success_rate ?? 0}% success rate). Keep that momentum going!`
